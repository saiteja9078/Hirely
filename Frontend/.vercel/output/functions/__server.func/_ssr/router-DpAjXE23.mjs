...
import { N as Bookmark, P as Bell, _ as Menu, g as MessageSquare, i as User, l as Sun, m as Moon } from "../_libs/lucide-react.mjs";
import { t as Toaster } from "../_libs/sonner.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/api-J34-O5Ts.js
...
async function authenticate(role, mode, data) {
	const result = await request(mode === "signin" ? `/login/${role === "hiring" ? "hiring-manager" : role}` : `/signup/${role === "hiring" ? "hiring-manager" : role}`, {
		method: "POST",
...
async function getCatalog() {
	const [skills, roles, industries, companies, departments] = await Promise.all([
		request("/api/catalog/skills").catch(() => []),
		request("/api/catalog/roles").catch(() => []),
		request("/api/catalog/industries").catch(() => []),
		listCompanies().catch(() => []),
		request("/api/catalog/departments").catch(() => [])
	]);
	return {
		skills,
		roles,
...
async function listJobs(query = "", filters = {}) {
	return (await request("/api/post-job/filter", {
		method: "POST",
		body: JSON.stringify({
			title: query || null,
...
async function listJobsPage(query = "", filters = {}, page = 0, size = 20) {
	const res = await request(`/api/post-job/filter?page=${page}&size=${size}`, {
		method: "POST",
		body: JSON.stringify({
			title: query || null,
			location: filters.location || null,
	...
	return {
		content: res.content.map(toJob),
		totalPages: res.totalPages,
		totalElements: res.totalElements
	};
}
async function getJob(id) {
	return toJob(await request(`/api/post-job/${id}`));
}
async function createJobPosting(data) {
	return request("/api/post-job", {
		method: "POST",
		body: JSON.stringify(data)
	});
}
async function getEmployerJobs() {
	return (await request("/api/post-job/mine")).map(toJob);
}
...
async function listCompanies() {
	return (await request("/api/companies")).map(toCompany);
}
...
async function getCompanyHiringManagers() {
	return request("/api/companies/me/hiring-managers");
}
async function createHiringManager(data) {
	return request("/signup/hiring-manager", {
		method: "POST",
		body: JSON.stringify(data)
	});
}
async function deleteHiringManager(id) {
	return request(`/api/companies/me/hiring-managers/${id}`, { method: "DELETE" });
}
async function getCurrentHiringManager() {
	return request("/api/hiring-managers/me");
}
async function getCompanyReviews(companyId) {
	return request(`/api/company-reviews/company/${companyId}`);
}
async function addCompanyReview(companyId, stars, text) {
	return request(`/api/company-reviews?companyId=${companyId}`, {
		method: "POST",
		body: JSON.stringify({
			stars,
			text
...
async function getCandidateSkills() {
	return request("/api/candidate-skills/me");
}
async function saveCandidateSkills(addExistingSkills, createNewSkills) {
	return request("/api/candidate-skills/me", {
		method: "POST",
		body: JSON.stringify({
			addExistingSkills: addExistingSkills.map((s) => ({
				id: s.skillId,
...
async function deleteCandidateSkill(skillId) {
	return request(`/api/candidate-skills/me/${skillId}`, { method: "DELETE" });
}
async function getCandidateExperiences() {
	return request("/api/candidate-experiences/me");
}
...
async function deleteCandidateExperience(id) {
	return request(`/api/candidate-experiences/me/${id}`, { method: "DELETE" });
}
async function getResumes() {
	return request("/api/files/upload/resume");
}
...
async function deleteResume(id) {
	return request(`/api/files/upload/resume/${id}`, { method: "DELETE" });
}
async function fetchResumeBlobUrl(id) {
	const token = typeof window !== "undefined" ? localStorage.getItem("hirely-token") : null;
	const response = await fetch(`${API_URL}/api/files/upload/resume/${id}/download`, { headers: token ? { Authorization: `Bearer ${token}` } : {} });
	if (!response.ok) throw new Error("Failed to download resume");
	const blob = await response.blob();
	return window.URL.createObjectURL(blob);
}
async function applyToJob(jobId, candidateId, coverLetter, resumeId, alerts) {
	return request(`/api/apply?jobId=${encodeURIComponent(jobId)}&candidateId=${candidateId}${coverLetter ? `&coverLetter=${encodeURIComponent(coverLetter)}` : ""}${resumeId ? `&resumeId=${resumeId}` : ""}${alerts ? `&alerts=true` : ""}`, { method: "POST" });
}
...
async function deleteJobApplication(applicationId) {
	return request(`/api/apply/${applicationId}`, { method: "DELETE" });
}
async function getApplicationDetails(applicationId) {
	return request(`/api/apply/${applicationId}`);
}
async function downloadApplicationResumeBlob(applicationId) {
	const token = typeof window !== "undefined" ? localStorage.getItem("hirely-token") : null;
	const response = await fetch(`${API_URL}/api/applications/${applicationId}/resume/download`, { headers: { ...token ? { Authorization: `Bearer ${token}` } : {} } });
	if (!response.ok) throw new Error("Failed to download resume");
...
//#endregion
//#region node_modules/.nitro/vite/services/ssr/assets/router-DpAjXE23.js
...
var styles_default = "/assets/styles-BUWLuYST.css";
...
function Header() {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("header", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [[...publicNav.filter((item) => {
				...
				children: item.label
			}, item.to)), !role && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
...
var $$splitComponentImporter$17 = () => import("./routes-DdG_M-mO.mjs");
...
var $$splitComponentImporter$16 = () => import("./dashboard-DO_fihod.mjs");
...
var $$splitComponentImporter$15 = () => import("./jobs-B_4UbSgN.mjs");
...
var $$splitComponentImporter$13 = () => import("./notifications-D3THN3i7.mjs");
...
var $$splitComponentImporter$12 = () => import("./profile-3vCpTZuC.mjs");
...
var $$splitComponentImporter$11 = () => import("./settings-BqHr6Ml-.mjs");
...
var $$splitComponentImporter$10 = () => import("./signin-C36IW2QO.mjs");
...
var $$splitComponentImporter$9 = () => import("./signup-JhKqEcxU.mjs");
...
var $$splitComponentImporter$8 = () => import("./applications._applicationId-BboPhXe8.mjs");
...
var $$splitComponentImporter$7 = () => import("./apply._jobId-BbRNsX5X.mjs");
...
var $$splitComponentImporter$6 = () => import("./companies.index-B-Pg284X.mjs");
...
var $$splitComponentImporter$5 = () => import("./companies._slug-tTb44F0N.mjs");
...
var $$splitComponentImporter$4 = () => import("./company.index-CMco_KDO.mjs");
...
var $$splitComponentImporter$3 = () => import("./company.post-job-Cpes3ebq.mjs");
...
var $$splitComponentImporter$2 = () => import("./company.profile-Bssbzxkz.mjs");
...
var $$splitComponentImporter$1 = () => import("./hiring.index-CzKbl2vg.mjs");
...
var $$splitComponentImporter = () => import("./hiring.jobs._jobId.applicants-BUQ-ZQcN.mjs");
