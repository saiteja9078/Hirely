...
export async function authenticate(
  ...
  const endpoint =
    mode === "signin"
      ? `/login/${role === "hiring" ? "hiring-manager" : role}`
      : `/signup/${role === "hiring" ? "hiring-manager" : role}`;
...
export async function getCatalog() {
  const [skills, roles, industries, companies, departments] = await Promise.all([
    request<CatalogItem[]>("/api/catalog/skills").catch(() => []),
    request<CatalogItem[]>("/api/catalog/roles").catch(() => []),
    request<CatalogItem[]>("/api/catalog/industries").catch(() => []),
    listCompanies().catch(() => []),
    request<CatalogDepartmentItem[]>("/api/catalog/departments").catch(() => []),
  ]);
...
export async function listJobs(
  ...
  const page = await request<{ content: ApiJobCard[] }>("/api/post-job/filter", {
    method: "POST",
    body: JSON.stringify({
      title: query || null,
...
export async function listJobsPage(
  ...
  const res = await request<{ content: ApiJobCard[], totalPages: number, totalElements: number }>(`/api/post-job/filter?page=${page}&size=${size}`, {
    method: "POST",
    body: JSON.stringify({
      title: query || null,
      location: filters.location || null,
...
export async function getJob(id: string) {
  return toJob(await request<ApiJob>(`/api/post-job/${id}`));
}

export async function createJobPosting(data: Record<string, unknown>) {
  return request<ApiJob>("/api/post-job", {
    method: "POST",
    body: JSON.stringify(data),
  });
}
...
export async function getEmployerJobs() {
  const list = await request<ApiJob[]>("/api/post-job/mine");
  return list.map(toJob);
}

export async function deleteJobPosting(id: string) {
  return request<void>(`/api/post-job/${id}`, { method: "DELETE" });
}

export async function createDepartment(name: string, companyId: number) {
  return request<CatalogDepartmentItem>("/api/departments", {
    method: "POST",
...
export async function listCompanies() {
  const list = await request<ApiCompany[]>("/api/companies");
  return list.map(toCompany);
}

export async function getCurrentCompany() {
  return request<ApiCompany>("/api/companies/me");
}
...
export async function getCompanyHiringManagers() {
  return request<HiringManagerMember[]>("/api/companies/me/hiring-managers");
}

export async function createHiringManager(data: Record<string, unknown>) {
  // Hit signup endpoint but do not save token, since company is calling it
  return request<{ token: string }>("/signup/hiring-manager", {
    method: "POST",
    body: JSON.stringify(data),
  });
}
...
export async function deleteHiringManager(id: number) {
  return request<void>(`/api/companies/me/hiring-managers/${id}`, { method: "DELETE" });
}

// Hiring Managers
export async function getCurrentHiringManager() {
  return request<HiringManagerMember>("/api/hiring-managers/me");
}

export async function updateHiringManager(
  id: number,
  data: {
    firstName?: string;
    lastName?: string;
  ...
  return request<HiringManagerMember>(`/api/hiring-managers/${id}`, {
    method: "PATCH",
    body: JSON.stringify(data),
  });
}
...
export async function getCompanyReviews(companyId: number) {
  return request<CompanyReview[]>(`/api/company-reviews/company/${companyId}`);
}

export async function addCompanyReview(companyId: number, stars: number, text: string) {
  return request<CompanyReview>(`/api/company-reviews?companyId=${companyId}`, {
    method: "POST",
...
export async function getCurrentCandidate() {
  return request<Candidate>("/api/candidates/me");
}
...
export async function getCandidateSkills() {
  return request<CandidateSkill[]>("/api/candidate-skills/me");
}
...
export async function deleteCandidateSkill(skillId: number) {
  return request<void>(`/api/candidate-skills/me/${skillId}`, {
    method: "DELETE",
  });
}
...
export async function getCandidateExperiences() {
  return request<CandidateExperience[]>("/api/candidate-experiences/me");
}
...
export async function addCandidateExperience(exp: {
  ...
  return request<void>("/api/candidate-experiences/me", {
    method: "POST",
    body: JSON.stringify({
      existingRoles: [],
      createRoles: [
...
export async function deleteCandidateExperience(id: number) {
  return request<void>(`/api/candidate-experiences/me/${id}`, {
    method: "DELETE",
  });
}
...
export async function getResumes() {
  return request<Resume[]>("/api/files/upload/resume");
}
...
export async function deleteResume(id: number) {
  return request<void>(`/api/files/upload/resume/${id}`, {
    method: "DELETE",
  });
}
...
export async function fetchResumeBlobUrl(id: number) {
  const token = typeof window !== "undefined" ? localStorage.getItem("hirely-token") : null;
  const response = await fetch(`${API_URL}/api/files/upload/resume/${id}/download`, {
...
export async function applyToJob(jobId: string, candidateId: number, coverLetter?: string, resumeId?: number, alerts?: boolean) {
  return request<void>(
    `/api/apply?jobId=${encodeURIComponent(jobId)}&candidateId=${candidateId}${
      coverLetter ? `&coverLetter=${encodeURIComponent(coverLetter)}` : ""
    }${resumeId ? `&resumeId=${resumeId}` : ""}${alerts ? `&alerts=true` : ""}`,
    { method: "POST" },
  );
}
...
export async function getMyApplications() {
  return request<UserApplication[]>("/api/applications/me");
}
...
export async function getJobApplicants(jobId: string) {
  return request<JobApplicant[]>(`/api/applications/job/${jobId}`);
}
...
export async function updateApplicationStatus(
  ...
  return request<UserApplication>(`/api/applications/${applicationId}`, {
    method: "PATCH",
...
export async function deleteJobApplication(applicationId: number) {
  return request<void>(`/api/apply/${applicationId}`, {
    method: "DELETE",
  });
}
...
export async function getApplicationDetails(applicationId: number) {
  return request<DetailedApplication>(`/api/apply/${applicationId}`);
}

export async function downloadApplicationResumeBlob(applicationId: number): Promise<Blob> {
  const token = typeof window !== "undefined" ? localStorage.getItem("hirely-token") : null;
  const response = await fetch(`${API_URL}/api/applications/${applicationId}/resume/download`, {
    headers: {
      ...(token ? { Authorization: `Bearer ${token}` } : {}),
    },
  });
...
export async function getNotifications() {
  return request<AppNotificationResponse[]>("/api/notifications/me");
}
